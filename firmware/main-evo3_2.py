...
class UIManager:
    def __init__(self, event_queue, state_machine):
        ...
        self.started = False # Flag di inizio cattura impulso *IRQ
        ...
        self.pulse_map = {
             (800, 1100): (0, 'sw_release'),
             (1800, 2100): (1, 'sel_1'),
             (2800, 3100): (1, 'sw_press'),
             (3800, 4100): (2, 'sel_2'),
             (5800, 6100): (3, 'sel_3'),
             (7800, 8100): (4, 'sel_4'),
             (9800, 10100): (5, 'sel_5'),
    ...
    def handle_start_irq(self, pin):
        ...
        if pin.value() == 0: # Il trigger per iniziare il conteggio è il fronte negativo
            maybe_time_start = time.ticks_us() # Catturo il "possibile" istante iniziale
            k = 0
            for _ in range(5): # Campiono N=5 volte il pin
                if pin.value() == 0: k += 1
            if k > 3 and not self.started:
                self.start_pulse = maybe_time_start
                self.started = True
        else:
            maybe_time_duration = time.ticks_diff(time.ticks_us(), self.start_pulse)
            k = 0
            for _ in range(100): # Campiono N=100 volte il pin
                if pin.value() == 1: k += 1
            if k > 90 and self.started: # Il trigger per terminare il conteggio è il fronte positivo
                self.pulse_duration = maybe_time_duration
                self.started = False
                if 800 <= self.pulse_duration <= 16100:
                    self._decode_pulse()
...
class BluetoothManager:
    def __init__(self, hardware, state_machine, sensor_data, event_queue):
        self.event_queue = event_queue
        self.state_machine = state_machine
        self.sensor_data = sensor_data
        self.hardware = hardware
        self.ble = bluetooth.BLE()
        self.peripheral = BLESimplePeripheral(self.ble, BT_N) # Il valore BT_N è letto da info.py
    ...
    def _on_rx(self, data):
        ...
        # self.hardware.start_pin.irq(None)
        # self.hardware.pos_sw.irq(None)
        ...
        # non serve: vengono riabilitati nel ciclo principale della GelatoMachine
    ...
    def _query_GetStatus(self):
        ...
        res = res + int(self.state_machine.pos_sw).to_bytes(1) # Posizione braccio motore
...
class GelatoMachine:   
    def __init__(self):
        ...
        self.ui_manager = UIManager(self.event_queue, self.state_machine)
        ...
        self.bluetooth = BluetoothManager(self.hardware, self.state_machine, self.sensor_data, self.event_queue)
    ...
    def _check_ota_update(self):
        ...
        try:
            self.state_machine.r, self.state_machine.g, self.state_machine.b = 0, 255, 0
    ...
    def run(self):
        ...
        while True:
            try:
                # Disabilita interrupt durante elaborazione critica
                self.hardware.start_pin.irq(None)
                self.hardware.pos_sw.irq(None)
                
                # Elabora eventi UI ---> se chiamata spostata all'interno di _decode_pulse vede tasto premuto per errore
                self.ui_manager.decode_events()
                
                # Processa gli eventi in coda elinimato:
                while not self.event_queue.is_empty():
                    self.state_machine.process_event(self.event_queue.get())

                # Cattura eventuali stati incoerenti e termina l'esecuzione per ispezione delle variabili (Thonny)
                # if self.state_machine.standby and self.state_machine.current_status == MachineStates.RUN_STD:
                ...
                time.sleep_ms(Config.UI_DECODE_DELAY)
