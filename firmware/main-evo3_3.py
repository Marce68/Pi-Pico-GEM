...
class UIManager:
    def __init__(self, event_queue, state_machine, hardware):
        ...
        self.pulse_map = {
             (900, 1100): (0, 'sw_release'),
             (1900, 2100): (1, 'sel_1'),
             (2950, 3050): (1, 'sw_press'),
             (3900, 4100): (2, 'sel_2'),
             (5900, 6100): (3, 'sel_3'),
             (7900, 8100): (4, 'sel_4'),
             (9800, 10100): (5, 'sel_5'),
    ...
    def handle_start_irq(self, pin):
        ...
        time.sleep_us(10)
        if pin.value() == 0 and not self.pulse_started: # Il trigger per iniziare il conteggio è il fronte negativo
            self.start_pulse = time.ticks_us() # Catturo l'istante iniziale
            self.pulse_started = True
        elif pin.value() == 1 and self.pulse_started: # Il trigger per terminare il conteggio è il fronte positivo
            self.pulse_duration = time.ticks_diff(time.ticks_us(), self.start_pulse) 
            # print(f"Impulso: {self.pulse_duration}") # Debug
            if 900 <= self.pulse_duration <= 16100:
                self._decode_pulse()
                self.decode_events()
            else:
                # print(f"Pulse out of range: {self.pulse_duration}") # Debug
                pass
    ...
    #         k = 0
    #         for _ in range(10): # Campiono N=10 volte il pin ogni 15us
    #             if pin.value() == 0:
    #                 k += 1
    #                 time.sleep_us(15) # Piccolo ritardo per evitare di campionare sempre lo stesso valore
    #         if k > 9 and not self.pulse_started:
    #             self.start_pulse = maybe_time_start
    #             self.pulse_started = True
    #     else:
    #         time.sleep_us(50) # Offset: la durata misurata è minore del valore nominale e varia da scheda a scheda ...
    #         maybe_time_duration = time.ticks_diff(time.ticks_us(), self.start_pulse) 
    #         k = 0
    #         for _ in range(10): # Campiono N=10 volte il pin ogni 50us
    #             if pin.value() == 1:
    #                 k += 1
    #                 time.sleep_us(50) # Piccolo ritardo per evitare di campionare sempre lo stesso valore
    #         if k > 9 and self.pulse_started: # Il trigger per terminare il conteggio è il fronte positivo
    #             self.pulse_duration = maybe_time_duration
    #             # print(f"Impulso: {self.pulse_duration}") # Debug
    #             if 900 <= self.pulse_duration <= 16100:
    #                 # self.hardware.start_pin.irq(None) # <<<<<<<<<<<< IRQ disable
    #                 self._decode_pulse()
    #                 self.decode_events()
    #                 # self.hardware.start_pin.irq(
    #                 #     trigger=Pin.IRQ_RISING | Pin.IRQ_FALLING,
    #                 #     handler=self.handle_start_irq
    #                 # ) # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< IRQ enable
    #             else:
    #                 print(f"Pulse out of range: {self.pulse_duration}") # Debug
    #             self.pulse_started = False
...
class BluetoothManager:
    ...
    def __init__(self, ui_manager, state_machine, sensor_data, event_queue):
        self.event_queue = event_queue
        self.state_machine = state_machine
        self.sensor_data = sensor_data
...
class GelatoMachine:   
    def __init__(self):
        ...
        self.bluetooth = BluetoothManager(self.ui_manager, self.state_machine, self.sensor_data, self.event_queue)
